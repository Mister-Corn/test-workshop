...
test('returns winner', () => {
  ...
  //
  const numOfTimesCalled = utils.getWinner.mock.calls.length
  // there's also a `toHaveLength`
  expect(numOfTimesCalled).toBe(2)
  // add another assertion that every time it was called
  // it was called with the right arguments: 'Ken Wheeler', 'Kent C. Dodds'
  // (Hint #3)
  utils.getWinner.mock.calls.forEach(argsArr =>
    expect(argsArr).toEqual(['Ken Wheeler', 'Kent C. Dodds']),
  )
